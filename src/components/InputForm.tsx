import type { UserInputs, SavingsBucket, CurrencyCode } from '../types';
import { DEFAULT_BUCKET, BUCKET_PRESETS, CURRENCIES } from '../types';
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Preferred Currency
              <InfoTooltip content="The currency used to display all monetary values. This is for display purposes only - no conversion is applied." />
            </label>
            <select
              value={inputs.currency}
              onChange={(e) => onChange({ ...inputs, currency: e.target.value as CurrencyCode })}
              className="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-blue-500 focus:border-blue-500"
            >
              {CURRENCIES.map((c) => (
